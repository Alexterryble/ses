...
try {
  $stmt = $conn->prepare("SELECT * FROM info_adicional WHERE solicitud_id = ?");
  $stmt->execute([$folio]);
  ...
  if ($data) {
    ...
    echo json_encode(['success' => false, 'message' => 'No se encontró información adicional para esta solicitud.']);
  }
